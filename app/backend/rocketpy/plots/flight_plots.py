...
class _FlightPlots:
    ...
    def combinedTrajectories(flightArray):
        ...
        max_z = max(flightArray[0].z[:, 1] - flightArray[0].env.elevation)
        max_x = max(flightArray[0].x[:, 1])
        min_x = min(flightArray[0].x[:, 1])
        max_y = max(flightArray[0].y[:, 1])
        min_y = min(flightArray[0].y[:, 1])
        maxTime = len(flightArray[0].z)
        for i in flightArray:
            subtractionNeeded = i.z[0][1]
            current_max_z = max(i.z[:, 1] - subtractionNeeded)
            ...
            if current_min_y < min_y:
                min_y = current_min_y
            current_maxTime = len(i.z)
            if current_maxTime > maxTime:
                maxTime = current_maxTime
        max_xy = max(max_x, max_y)
        min_xy = min(min_x, min_y)
        ...
        for i in range(maxTime):
            theSum = 0
            countOfCurrentFlights = 0
            for j in flightArray:
                if (len(j.x) > i):
                    theSum += j.x[i][1]
                    countOfCurrentFlights += 1
            xPos.append(theSum / countOfCurrentFlights)
        ...
        for i in range(maxTime):
            ...
            for j in flightArray:
                if (len(j.y) > i):
                    theSum += j.y[i][1]
                    countOfCurrentFlights += 1
            yPos.append(theSum / countOfCurrentFlights)
        ...
        for i in range(maxTime):
            ...
            for j in flightArray:
                if (len(j.z) > i):
                    subtractionNeeded = j.z[0][1]
                    theSum += j.z[i][1]  - subtractionNeeded
                    countOfCurrentFlights += 1
            zPos.append(theSum / countOfCurrentFlights)

        line1, = ax1.plot(
            xPos, yPos, zs=0, zdir="z", linestyle="--", label='Change In Ground Position'
        )
        line2, = ax1.plot(
            xPos,
            zPos,
        ...
        ax1.set_ylabel("Y - North (m)")
        ax1.set_zlabel("Z - Altitude Above Ground Level (m)")
        ax1.set_title("Average of All Trajectories")
